function rightAccordion(measurements) {
  _.forEach(measurements, function(value, source) {
    ...
    $count.html(
      " (Selected: " +
        $count.attr("data-selected") +
        " of " +
        $count.attr("data-total") +
        ")"
    );
...
function loadMeasurements(datasource, input) {
  ...
  annotations.forEach(function(text) {
    ...
    if (
      parseInt(values[getRandom(0, values.length - 1)]) &&
      values.length > 5
    ) {
      ...
      values.forEach(function(anno) {
        ...
        label.innerHTML =
          anno +
          '<div class="ui mini circular horizontal label"> ' +
          allCounts[anno] +
          "</div>";
        fields.appendChild(field);
        field.appendChild(checkbox);
  ...
  $(".ui.search").search({
    ...
    cache: false,
    onResults: results => {
      searchFilter(results.results, searchList);
    }
  });
  ...
  Object.keys(ranges).forEach(function(ids) {
    $(ranges[ids][1]).change(function() {
      filter(
        [parseInt($(ranges[ids][1]).val()), parseInt($(ranges[ids][2]).val())],
        fTracker[ids.split("-")[0]],
        true,
        measurements
      );
    });
