...
import './chart-data-behavior.js';
import './chart-add-behavior.js';
import './chart-workspace-behavior.js';
import './chart-grid-behavior.js';
import './chart-draggable-behavior.js';
import './epiviz-genes-track.js';
import './epiviz-ideogram-track.js';
...
$_documentContainer.innerHTML = `<dom-module id="epiviz-navigation">
    <!--<link rel="import" href="../epiviz-imports/epiviz-common-css.html">-->
    <template>
...
class EpivizNavigation extends EpivizChartDraggableBehavior(EpivizChartGridBehavior(EpivizChartWorkspaceBehavior(EpivizChartDataBehavior(EpivizChartAddBehavior(EpivizChartBehavior(PolymerElement)))))) {

    static get is() { return 'epiviz-navigation'; }
    ...
    constructor() {
        ...
        this.addEventListener('iron-resize', e => {
            this._debouncer = Debouncer.debounce(this._debouncer, timeOut.after(100), () => this._onResize(e));
        });
    ...
    connectedCallback() {
        ...
        if (!origCollapsed) {
            self._observer = new FlattenedNodesObserver(self.$.chartNav, (info) => {
                // self._observer =
    ...
    ready() {
        ...
        else { self.transformFunc = self.epivizToJSON; }
    ...
    _onResize(e) {
        ...
        // }
        let navChildren =
            FlattenedNodesObserver.getFlattenedNodes(self.$.chartNav).filter(n => n.nodeType === Node.ELEMENT_NODE);
    ...
    _rangeUpdate(newVal, oldVal) {
        ...
        if (!self.collapsed && oldVal) {
            ...
            let navChildren =
                FlattenedNodesObserver.getFlattenedNodes(self).filter(n => n.nodeType === Node.ELEMENT_NODE && n.nodeName.indexOf("EPIVIZ") !== -1);
            var numChildren = navChildren.length;
            if (numChildren > 0) { self.set('_isEmpty', true); }
            for (var index = 0; index < numChildren; index++) {
                var currentChild = navChildren[index];
        ...
        self.dispatchEvent(new CustomEvent('navigationRangeUpdate',
            {
                detail: {
                    plotId: self.plotId,
                    range: self.range
    ...
    _updateStrRange(e) {
        ...
        if (chr in this.seqInfo) {
            ...
            dom(this.root).appendChild(toast);
            toast.show();
        }
    }
    ...
    cloneNavigation(event) {
        ...
        dom(self.parentNode).appendChild(navElem);
        let navChildren =
            FlattenedNodesObserver.getFlattenedNodes(self).filter(n => n.nodeType === Node.ELEMENT_NODE);
        var numChildren = navChildren.length;
        if (numChildren > 0) { self.set('_isEmpty', true); }
        for (var index = 0; index < numChildren; index++) {
            ...
            dom(navElem).appendChild(childElem);
        }
    }
