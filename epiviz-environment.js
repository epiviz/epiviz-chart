...
class EpivizEnvironment extends EpivizChartDraggableBehavior(EpivizChartWorkspaceBehavior(EpivizChartDataBehavior(EpivizChartAddBehavior(PolymerElement)))) {
  static get template() {
    return html`
        <style>
            :host {
...
`;
  ...
  connectedCallback() {
      ...
      self._observer = new FlattenedNodesObserver(this.$.chartEnv, (info) => {
          info.addedNodes.forEach(function (node) {
              node._parentContainer = self;
              if (node.nodeName === "EPIVIZ-NAVIGATION") {
                  self._getElementSeqInfo(node, dom(self));
                  self.push("navigationRegions", { "plotId": node.plotId, "range": node.range });
                  let navChildren =
                      FlattenedNodesObserver.getFlattenedNodes(node).filter(n => n.nodeType === Node.ELEMENT_NODE)
  ...
  _onResize() {
      var self = this;
      let navChildren =
          FlattenedNodesObserver.getFlattenedNodes(this.$.chartEnv).filter(n => n.nodeType === Node.ELEMENT_NODE);
